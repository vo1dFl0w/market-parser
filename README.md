...
1. Установите локальный Chromium и зайдите на сайт `https://kuper.ru` для получения куки и решения капчи (если появится).

2. Подготовьте `.env` на основе `.env.example` и `config.yaml`.

3. Соберите и запустите приложение через Makefile:
...
4. Пример HTTP-запроса к API (парсинг категории):
...
* Рекомендую для дальнейшей стабильной работы парсера самостоятельно зайти на сайт `https://kuper.ru` для получения куки и решения капчи (если появится).

1. Подготовьте `.env` на основе `.env.example` и `config.yaml`. Установите путь до вашего локального профиля chromium.
...
* Если приложение не подключается к браузеру — проверьте `BROWSER_WS_URL` и что Chromium запущен с remote debugging.
* Для проверки прокси попробуйте запустить Chromium локально с параметром прокси и протестировать доступ к сайту вручную.
* Логи пишутся в консоль — посмотрите вывод `make run` или `docker-compose logs`.
